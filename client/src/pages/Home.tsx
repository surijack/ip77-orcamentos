...
function ProposalPreview({ items, quoteNumber, emissionDate, seller, customer, total, onGenerate }: { items: QuoteItem[]; quoteNumber: string; emissionDate: string; seller: SellerData; customer: CustomerData; total: string; onGenerate: () => void }) {
  return <div className="rounded-2xl border border-slate-200 bg-white shadow-[0_16px_42px_rgba(23,63,107,0.08)] print:shadow-none print:border-0">
    <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 print:hidden"><div><div className="text-sm font-bold text-[#173f6b]">Prévia da proposta</div><div className="mt-1 text-xs text-slate-400">Documento profissional com a identidade da IP77</div></div><button onClick={onGenerate} className="flex items-center gap-2 rounded-lg bg-[#173f6b] px-3.5 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-[#0f3157]"><Download size={14} /> Gerar PDF</button></div>
