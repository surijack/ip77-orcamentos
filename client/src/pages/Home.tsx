...
function ProposalPreview({ items, quoteNumber, emissionDate, total, onPrint }: { items: QuoteItem[]; quoteNumber: string; emissionDate: string; total: string; onPrint: () => void }) {
  return <div className="rounded-2xl border border-slate-200 bg-white shadow-[0_12px_32px_rgba(28,61,92,0.05)] print:shadow-none print:border-0">
    ...
    <div className="proposal-print-area p-6 sm:p-8">
      <div className="flex items-start justify-between gap-6 border-b-2 border-[#12a3a5] pb-5"><div className="flex min-w-0 items-center gap-3"><img src={LOGO} alt="IP77" className="h-12 w-auto shrink-0" /><div className="min-w-0"><div className="text-xl font-black tracking-tight text-[#173f6b]">PROPOSTA COMERCIAL</div><div className="mt-1 text-[9px] leading-4 text-slate-500"><span className="font-bold text-slate-600">IP77</span><span className="mx-1.5 text-slate-300">·</span><span>CNPJ {IP77_COMPANY.cnpj}</span><span className="mx-1.5 text-slate-300">·</span><span>{IP77_COMPANY.address}</span><span className="mx-1.5 text-slate-300">·</span><span>{IP77_COMPANY.neighborhood} / {IP77_COMPANY.state}</span></div></div></div><div className="min-w-[145px] shrink-0 rounded-lg bg-[#f1f7fa] px-4 py-3 text-right"><div className="text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400">Cotação</div><div className="mt-1 text-sm font-black text-[#173f6b]">{quoteNumber || "—"}</div><div className="mt-2 text-[9px] text-slate-500"><span className="font-bold text-slate-600">Emissão:</span> {emissionDate || "—"}</div><div className="mt-0.5 text-[9px] text-slate-500"><span className="font-bold text-slate-600">Validade:</span> 5 dias</div></div></div>
