...
function ProposalPreview({ items, quoteNumber, emissionDate, customer, total, onPrint }: { items: QuoteItem[]; quoteNumber: string; emissionDate: string; customer: CustomerData; total: string; onPrint: () => void }) {
  return <div className="rounded-2xl border border-slate-200 bg-white shadow-[0_12px_32px_rgba(28,61,92,0.05)] print:shadow-none print:border-0">
    ...
    <div className="proposal-print-area p-6 sm:p-8">
      ...
      <div className="mt-5 grid gap-3 border-t border-slate-200 pt-4 sm:grid-cols-2"><div className="rounded-lg border border-[#e3b400] bg-[#fff8d9] p-3 text-[9px] leading-4 text-slate-700"><div className="mb-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-[#9a7200]">Dados da IP77</div><div><b>CNPJ:</b> {IP77_COMPANY.cnpj}</div><div><b>Endereço:</b> {IP77_COMPANY.address}</div><div><b>Bairro/UF:</b> {IP77_COMPANY.neighborhood} / {IP77_COMPANY.state}</div></div><div className="rounded-lg border border-[#46a56b] bg-[#eaf8ef] p-3 text-[9px] leading-4 text-slate-700"><div className="mb-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-[#2b8050]">Dados do cliente comprador</div><div><b>Nome:</b> {customer.name || "Não identificado"}</div><div><b>CPF/CNPJ:</b> {customer.document || "Não identificado"}</div><div><b>Endereço:</b> {customer.address || "Não identificado"}</div><div><b>Cidade/UF:</b> {customer.cityState || "Não identificado"} {customer.phone && <><span className="ml-2"><b>Telefone:</b> {customer.phone}</span></>}</div></div></div>
